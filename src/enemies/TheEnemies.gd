extends Node

# The poor guys
onready var SmallAsteroid = preload("res://enemies/SmallAsteroid.tscn")
onready var SmallSaucer = preload("res://enemies/SmallSaucer.tscn")
onready var SmallSaucerSilver = preload("res://enemies/SmallSaucerSilver.tscn")
onready var SmallSaucerGold = preload("res://enemies/SmallSaucerGold.tscn")

# Bosses
onready var SaucerBoss = preload("res://bosses/SaucerBoss.tscn")
#onready var SaucerBossSilver = preload("res://bosses/SaucerBossSilver.tscn")
#onready var SaucerBossGold = preload("res://bosses/SaucerGold.tscn")
